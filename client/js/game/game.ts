...
export class Renderer {
	...
	mouseHandle: MouseHandler = new MouseHandler();
	...
	constructor({
		...
		this.mouseHandle.addEventListener('scroll', async (e) => {
			const mouseBefore = this.convertMouse(
				[this.mouseHandle.mouseDatum.x, this.mouseHandle.mouseDatum.y],
				false
			);

			this.zoom = e.direction == 'up' ? this.zoom / 1.04 : this.zoom * 1.04;

			const mouseAfter = this.convertMouse(
				[this.mouseHandle.mouseDatum.x, this.mouseHandle.mouseDatum.y],
				false
			);
		...
		this.keyHandle.on('stop', async (data) => {
			if (this.keyHandle.currentKeys.mod) stop();
		});
	...
	convertMouse(pos: Point, round = true): Point {
		const scaleFactor = this.tileSize * this.zoom;
		let out = pos;
	...
	draw() {
		...
		if (this.prevTimes.length > 60) this.prevTimes.shift();
		...
		const draws: [dx: number, dy: number, tile: Tile][] = [];
		...
		for (const dx of range(bounds[0][0], bounds[1][0], 1, false)) {
			for (const dy of range(bounds[0][1], bounds[1][1], 1, false)) {
				if (lenPoint([dx, dy]) < this.player.viewRange) {
					...
					if (final != null && final.drawCount > 0) draws.push([dx, dy, final]);
				}
			}
		}
		...
		while (draws.length > 0) {
			...
			for (let tile of draws) {
				if ('draw' + pass in tile[2]) {
					...
					ctx.restore();
				} else draws.splice(draws.indexOf(tile), 1);
			}
		}
